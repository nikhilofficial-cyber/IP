...
- Precedence of operators is the order or priority in which operators are evaluated in an expression. Operators with higher precedence are executed before operators with lower precedence. If two operators have the same precedence, the associativity of the operators decides the order of evaluation.

### Important Precedence Order (High → Low)
...
## Statement
- A statement is a single instruction or command in a Python program that tells the computer to perform a specific task. Statements are executed one by one by the Python interpreter.
- Examples 
    ```python
    x = 5
    print(x)
...
## INPUT AND OUTPUT
### Input
- Input is the data or information entered by the user into a program for processing. In Python, the input() function is used to take input from the user.
- Example
    ```python
...
### Output
- Output is the result produced by a program after processing the input. In Python, the print() function is used to display output on the screen.
- Example
    ```python
    print("Still Alive!!!")
    ```
...
## Type Conversion
- Type Conversion is the process of changing the data type of a value or variable from one type to another. In Python, type conversion can be implicit (automatic) or explicit (manual).

### 1. Implicit Conversion
- Implicit Type Conversion is the automatic conversion of one data type into another by Python during an operation. Python performs this conversion without any instruction from the programmer to avoid data loss and make the expression valid. It's also known as Coercion .

#### Key Points
- Done automatically by Python.
- No conversion functions are used.
- Helps avoid data loss.
- Makes expressions with different data types valid.

<br>

### 2. Explicit Conversion
- Explicit Type Conversion is the manual conversion of one data type into another by the programmer using Python's built-in conversion functions such as int(), float(), str(), and bool(). It's also known as Type Casting .
- Syntax
    ```python
...
- Uses conversion functions like int(), float(), str(), and bool().
- Gives the programmer control over data types.
- Useful when automatic conversion is not possible.

<br><br>

Difference Between Implicit and Explicit Conversion (Very Important)

| **Implicit Conversion**           | **Explicit Conversion**                               |
| --------------------------------- | ----------------------------------------------------- |
| Also known as **Coercion**.       | Also known as **Coercion**.                      |
| Done automatically by Python.     | Done manually by the programmer.                      |
| No conversion function is used.   | Uses functions like `int()`, `float()`, `str()`, etc. |
| Prevents data loss automatically. | Gives full control over data type conversion.         |
| Example: `5 + 2.5 → 7.5`          | Example: `int("25") → 25`                             |

<br><br><br>

## Debugging
- Debugging is the process of finding, identifying, and correcting errors (bugs) in a program so that it runs correctly and produces the expected output.Error occuring can be categorised as : simple errors, logical errors and Runtime errors.

### Types of Errors (Debugging Helps Fix Them)
#### 1. Syntax Errors
- A Syntax Error is an error that occurs when the rules (syntax) of the Python language are not followed. The program cannot run until the syntax error is corrected.
- Example:
    ```python
    if x > 10
    print(x)
    # Error: Missing : after the if statement (if x > 10:).
    ```

#### 2. Logical Error
- A Logical Error is an error in the logic of a program. The program runs successfully without showing any error, but it produces the wrong output.
- Example:
    ```python
    length = 5
    breadth = 4
    area = length + breadth
    print(area)
    # This is incorrect because the area of a rectangle should be calculated using multiplication (area = length * breadth).
    ```
#### 3. Runtime Error
- A Runtime Error is an error that occurs while the program is running. The program starts executing but stops when it encounters an invalid operation.
- Example:
    ```python
    a = 10
    b = 0
    print(a / b)
    # This will give an ZeroDivisionError: division by zero  
    ```
...
| -------------------------------------- | ------------------------------------------ | ------------------------------------------------------- |
| Occurs due to incorrect Python syntax. | Occurs while the program is running.       | Occurs due to incorrect program logic.                  |
| Program does not start.                | Program starts but stops during execution. | Program runs completely.                                |
| Easy to detect.                        | Detected during execution.                 | Difficult to detect.                                    |
| Example: Missing `:`                   | Example: Division by zero.                 | Example: Using `+` instead of `*` for area calculation. |
